    ...
    public class CCustomer : CPerson
    {
        ...
        public bool Insert()
        {
            try
            {
                ...
                string sSQL = "INSERT INTO tblCustomers select '" +
                                Id.ToString() +
                                "', '" + Ssn +
                                "', '" + FirstName +
                                "' , '" + LastName +
        ...
        public int Update()
        {
            try
            {
                ...
                string sSQL = "UPDATE tblCustomers SET Ssn = '" + Ssn + "'," +
                              " FirstName = '" + FirstName + "'," +
                              " LastName = '" + LastName + "'," +
        ...
        public int Delete()
        {
            try
            {
                ...
                string sSQL = "DELETE FROM tblCustomers " +
                              " WHERE CustomerId = '" + Id + "'";
        ...
        {
            // Get one Customer's deposit data from the database.
            try
            {
                CDatabase oDb = new CDatabase();
                depositlist = new List<CDeposit>();

                string sSQL = "SELECT * FROM tblDeposits where CustomerId = '" + Id + "'";
                ...
                {
                    CDeposit oDeposit = new CDeposit();
                    oDeposit.DepositId = Convert.ToInt32(odr["DepositId"]);
                    oDeposit.DepositAmount = Convert.ToInt32(odr["DepositAmount"]);
                    oDeposit.DepositDate = Convert.ToDateTime(odr["DepositDate"]);

                    depositlist.Add(oDeposit);
                }
          ...
          public void GetWithdrawlData()
        {
            // Get one Customer's withdrawl data from the database.
            try
            {
                CDatabase oDb = new CDatabase();
                Withdrawllist = new List<CWithdrawl>();

                string sSQL = "SELECT * FROM tblWithdrawls where CustomerId = '" + Id + "'";
                ...
                foreach (DataRow odr in oDT.Rows)
                {
                    CWithdrawl oWithdrawl = new CWithdrawl();
                    oWithdrawl.WithdrawlId = Convert.ToInt32(odr["WithdrawlId"]);
                    oWithdrawl.WithdrawlAmount = Convert.ToInt32(odr["WithdrawlAmount"]);
                    oWithdrawl.WithdrawlDate = Convert.ToDateTime(odr["WithdrawlDate"]);

                    Withdrawllist.Add(oWithdrawl);
                }
